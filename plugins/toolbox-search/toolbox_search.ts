...
import { BlockSearcher } from "./block_searcher.ts";
...
export class ToolboxSearchCategory extends Blockly.ToolboxCategory {
  ...
  private blockSchemas: Map<string, Blockly.utils.toolbox.BlockInfo> =
    new Map();
  ...
  private getAvailableBlocks(
    ...
    if ("contents" in schema) {
      ...
      if ("type" in schema && schema.type) {
        ...
        this.blockSchemas.set(schema.type, schema); // <-
      }
    }
  }
  ...
  private matchBlocks() {
    ...
    this.flyoutItems_ = query
      ? this.blockSearcher.blockTypesMatching(query).map((blockType) => {
          ...
          // this includes the whole block schema, including shadow blocks for example
          return this.blockSchemas.get(blockType) as any;
        })
      : [];
